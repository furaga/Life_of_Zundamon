...
def load_center_images(center_dir: Path):
    all_img_paths = list(center_dir.glob("*.png"))
    for img_path in all_img_paths:
        img = imread_safe(str(img_path))
        feat = get_clip_features(img)
        feat /= np.linalg.norm(feat)
        center_dict_[img_path.stem] = feat
    print("Loaded", len(center_dict_), "center images.")
...
def init(root_dir: Path):
    ...
    load_center_images(root_dir / "center")
...
def detect_place(img):
    ...
    ls = []
    for item_name, ref_feat in place_dict_.items():
